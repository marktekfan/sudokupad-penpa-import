...
export class FakeDoc {
		...
		if (!elem) {
			elem = {
				...
				classList: {
					classes: {},
					add: (c: string) => {
						elem.classList.classes[c] = 1;
					},
					remove: (c: string) => {
						delete elem.classList.classes[c];
					},
					contains: (c: string) => elem.classList.classes[c],
				},
				getElementsByClassName: (c: string) =>
					Object.keys(this._elem)
						.filter(id => Object.keys(this._elem[id].classList.classes).includes(c))
						.map(id => this._elem[id]),
				addEventListener: (_e) => {},
				checked: undefined,
			};
			this._elem[id] = elem;
	...
	getValues() {
		let doc: Dictionary<string | boolean> = {};
