...
import { Puzzle_square as _Puzzle_square, Puzzle_sudoku as _Puzzle_sudoku, Puzzle_kakuro as _Puzzle_kakuro, Stack } from './penpa-puzzle';
...
export const PenpaGeneral = function (document) {
	...
	const Puzzle_square = new Proxy(_Puzzle_square, {
		construct: (target, args) => new target(document, ...args)
	});
	const Puzzle_sudoku = new Proxy(_Puzzle_sudoku, {
        construct: (target, args) => new target(document, ...args)	
    });
	const Puzzle_kakuro = new Proxy(_Puzzle_kakuro, {
        construct: (target, args) => new target(document, ...args)
	});
	...
	//
	// TODO: Capture from ./penpa-legacy/general.js
	//
	...
	function advancecontrol_off(loadtype) {
		...
		if (user_choices.indexOf('Special') === -1 && user_choices.indexOf('Thermo') === -1 && user_choices.indexOf('Sudoku Arrow') === -1) {
		...
		if (user_choices.indexOf('Sudoku Normal') === -1 && user_choices.indexOf('Sudoku Corner') === -1 && user_choices.indexOf('Sudoku Centre') === -1) {
	...
	function load(urlParam, type = 'url', origurl = null) {
		...
		if (pu.user_tags.length === 0) {
			...
			if (title_words) {
				...
				} else if ((sudoku_index === 1 || sudoku_index === 2) && allow_genres.includes(title_words[0].toLowerCase())) {
		...
		if (valid_replay) {
			...
			if (paramArray.q) {
				...
				pu.puzzleinfo.totalMS =
					(+solvetime[0] * 24 * 60 * 60 + +solvetime[1] * 60 * 60 + +solvetime[2] * 60 + +solvetime[3] + +solvetime[4] * 0.1) * 1000;
			}
		}
	}
	...
	function decode_puzzlink(url) {
		...
		var info_edge, info_number, info_obj, size, puzzlink_pu, row_ind, col_ind, cell, value, corner_cursor, number_style, map_genre_tag;
		...
		switch (type) {
			...
			case 'tapaloop':
				...
				info_number = type === 'tapa' ? puzzlink_pu.decodeTapa() : puzzlink_pu.decodeTapaLoop();
